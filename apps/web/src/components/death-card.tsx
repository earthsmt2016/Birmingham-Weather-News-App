...
import type { DeathArticle } from "@workspace/shared-api-zod";
  ...
  const handleShare = async (e: MouseEvent) => {
    ...
    if (navigator.share) {
      try {
        ...
        reportClientError("Native death article share failed", error);
      }
    }
    try {
      await navigator.clipboard.writeText(article.link);
    } catch (error) {
      reportClientError("Clipboard death article share fallback failed", error);
    }
  };
