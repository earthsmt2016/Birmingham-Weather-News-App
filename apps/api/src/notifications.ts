...
import { configureWebPush, hasVapidConfig } from "./lib/vapid";
...
interface NotificationWeatherResponse {
...
async function fetchWeatherForSub(sub: { latitude: string; longitude: string }): Promise<WeatherFetchResult | null> {
  ...
  try {
    ...
    const data = await response.json() as NotificationWeatherResponse;
    const weatherData: CurrentWeatherRaw = data.current;
    const daily = data.daily;
    const dailyData: DailyWeatherRaw[] = daily?.time?.map((_, i) => ({
    ...
    logger.warn(`Weather notification fetch failed: ${errMsg(error)}`);
    return null;
  }
}
...
async function fetchNewsArticles(locationName: string, count: number): Promise<NewsArticleItem[]> {
  ...
  try {
    ...
    const results = await Promise.allSettled(feeds.map(async (feed) => {
      try {
        ...
        return (parsed.items || []).slice(0, 5).map(item => ({
          ...
          logger.warn(`Notification news feed failed (${feed.source}): ${errMsg(error)}`);
          return [];
        }
      }));
    ...
    logger.warn(`Notification news fetch failed: ${errMsg(error)}`);
    return [];
  }
}
